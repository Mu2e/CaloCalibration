...
void SourceFitter::FitCrystal(TH1F* h_spec, TString opt, int crystalNo,  TTree *covar, Float_t &fpeak, Float_t &fsigma, Float_t &chiSq){
    ...
    RooFormulaVar fullRes("fullRes","full peak resolution","sqrt(pow(A/pow(fullPeak/1000,0.25),2))+B**2+pow(C/(fullPeak*0.0625),2)", RooArgSet(A, fullPeak, B, C));
    RooFormulaVar fullWidth("fullWidth", "width of the full peak", "(fullPeak*0.0625)*fullRes",RooArgSet(fullPeak,fullRes));
    ...
    RooFormulaVar scdRes("scdRes","second peak resolution","sqrt(pow(A/pow((scdEsPeak*0.0625)/1000,0.25),2))+B**2+pow(C/(scdEsPeak*0.0625),2)", RooArgSet(A, scdEsPeak, B, C));
